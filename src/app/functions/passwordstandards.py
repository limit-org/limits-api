...
def CheckPassword(plainTextPass, username):
    if len(plainTextPass) >= 257:  # check if too long
        return {
            ...
            "ui": "Your password is too long (over 256 characters)."
        }

    if len(plainTextPass) <= 7:  # equal to or less than 7 characters # check if too short
        return {
            "err": "Password not long enough.",
            "ui": "Your password needs to be longer (8 characters or more)."
        }
    ...
    if not digits:  # if no digits, do nothing if there are digits.
        return {
            "err": "No digits.",
            "ui": "There are no numbers in your password. At least 1 is required."
        }

    # Check if the password contains the username
    if username in plainTextPass:
        return {
            "err": "Username in password.",
            "ui": "Your password cannot contain your username."
        }
